...
const ERC20Mock = artifacts.require('ERC20MutableMock');

var chai = require("chai");
chai.should();
chai.use(require('chai-as-promised'));

contract('ERC20Mutatable', function ([_, initialHolder, recipient, anotherAccount]) {
  ...
  describe('_setTotalSupply', function () {
    it('sets the correct number of tokens', async function () {
      var supply = new BN(getRandomInt(1000000));
      await this.token.setTotalSupply(supply);
      (await this.token.totalSupply()).should.be.bignumber.equal(supply);
    });
  });

  describe('_setBalance', function () {
    it('sets the correct number of tokens', async function () {
      var balance = new BN(getRandomInt(1000000));
      await this.token.setBalance(anotherAccount, balance);
      (await this.token.balanceOf(anotherAccount)).should.be.bignumber.equal(balance);
    });
  });
